...
- `dma_board.c` — main firmware: init, Core 0 loop (USB CDC console poll, stuck detector, stack canary, HardFault capture, log flushing)
- `console/` — management console: `tui.c` (micro_tui REPL + full-screen menu over USB CDC), `tui_cmds.c` (REPL command table), `console_ops.{c,h}` (core-1 execution mailbox for mount/eject/ls/wifi/peek/status), `mgmt_con.c` (the SAME command table bound a second time over the host control channel; served from core 0's main loop), `vtasm_dma_blob.h` (checked-in VTASM download blob, generated by `victor_client/build.sh`), `usb_descriptors.c` + `tusb_config.h` (TinyUSB CDC). Uses the external `micro_tui` toolkit at `/root/sync/micro_tui`.
- `pico_victor/mgmt_chan.{c,h}` — host control channel core (portable SPSC, host-tested): the Victor detects/drives the card through three virtual registers in the 0xEF300 window. Ported from v9k_flop's `via_chan`.
- `victor_client/` — Victor-side VTASM terminal (`viaterm_dma.asm`) + its vendored deps (`keyboard.asm`, `NORMAL.CHR`), `gen_blob.py`, `build.sh` (nasm) → regenerates `console/vtasm_dma_blob.h`. Also builds `vtasm_dma.com` (DOS test build). `dmaterm.c` = DOS terminal for the management console (near-verbatim port of viclibc2/viaterm.c, channel on 0xEF30:0x40/0x50/0x60), built with OpenWatcom via `build_dos.sh` → `DMATERM.EXE`.
...
- **Diag UART**: `uart0`, **TX GPIO0 / RX GPIO33, 115200 baud (was 230400)** — **logs only** now (init banner, SD scan, SASI/stuck/timing dumps, HardFault capture). The old single-char command console (`h`/`t`/`p`/`i`/`c`/`f`/`a`/`r`) and the `F` FujiNet bench shell were removed; `p`/`t`/`i` dumps still fire automatically from the stuck detector.
- **Interactive console**: **USB CDC (micro_tui)** on the card's USB-C — a two-mode console (raw TinyUSB, `stdio_usb` stays off). Default is a line REPL (prompt `v9k> `; every reply ends `OK` or `ERR <reason>`); `menu` opens a full-screen ANSI menu. Commands: `ls` (SD + FujiNet images, `sd:`/`fuji:` prefixed), `status` (per target 0-7: name/backend/mounted/sectors), `mount <t> <[sd:|fuji:]file>` (bare = sd:), `eject <t>`, `peek <t> <lba>` (512-byte hex dump), `wifi` / `wifi scan` / `wifi <ssid> <pass>`, `diag` (UART liveness ping + DRDY + per-target backends), `menu`, `help`. Console handlers on core 0 defer all storage/SPI work to core 1 through the console-ops mailbox (`console/console_ops.c`); pure memory/GPIO getters run inline. See `console/tui.c`, `console/tui_cmds.c`, and `MGMT_CONSOLE_PLAN.md`.
...
- **Host control channel** (`mgmt_chan` + `mgmt_con`): three virtual registers in the 0xEF300 window let the Victor detect and drive the management console from the boot ROM / DOS. After `dma_mask_offset`: **0x40 CMD** (write = push command byte; read = complement echo of last write, idle 0xFF), **0x50 STATUS** (read = `0xD0 | flags`: b3 OVERFLOW / b2 CMD_FULL / b1 RESP_READY / b0 RESP_PHASE; write = soft reset), **0x60 RESP** (read = response byte; write = ack/advance). Signature nibble **0xD0** (the flop card uses 0xB0 — clients tell them apart). Detect from the Victor: write 0x55 to 0x40 and read back 0xAA, write 0xAA read back 0x55, then check `STATUS & 0xF0 == 0xD0`. Register-cache cells 0x40/0x50/0x60 are the channel's read shadows; ISR write path handles these synchronously (`register_irq_handlers.c` — complement echo / reset flag / ack bump, O(1), no defer queue). Command 0x1C streams the VTASM download blob (`A5 5A len_lo len_hi <payload> sum16`). Menu-exit CSI `ESC[86q`.
...
- **Host control-channel unit test**: `test/host/` — `make -C test/host check` builds and runs `test_mgmt` (1024-byte FIFO) and `test_mgmt_tiny` (8-byte FIFO) against `pico_victor/mgmt_chan.c` (plain gcc, no Pico SDK): detection probe, round trip, small-FIFO no-loss, OVERFLOW bound, reset, framed 0x1C blob download, and reset-mid-send. Ported from v9k_flop's `test_via.c`.
