...
- `victor_client/` — Victor-side VTASM terminal (`viaterm_dma.asm`) + its vendored deps (`keyboard.asm`, `NORMAL.CHR`), `gen_blob.py`, `build.sh` (nasm) → regenerates `console/vtasm_dma_blob.h`. Also builds `vtasm_dma.com` (DOS test build).
